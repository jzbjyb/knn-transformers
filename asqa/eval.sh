...
RESULTS_PATH=$1 # path to result jsonl file
RESULTS_PATH_KV=$RESULTS_PATH.kv # path to kv json file
EXP_NAME=$2 # name of your experiment
OUTPUT_DIR=./results/${EXP_NAME}

mkdir -p ${OUTPUT_DIR}
python ../prep.py --task jsonl_to_keyvalue --inp $RESULTS_PATH --out $RESULTS_PATH_KV

python convert_to_roberta_format.py  \
  --asqa ./dataset/ASQA.json \
  --predictions $RESULTS_PATH_KV  \
  --split dev \
