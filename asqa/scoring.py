...
def score_qa_accuracy(predictions, asqa, target_keys=None):
  ...
  for key, instance in asqa.items():
    ...
    for idx, qa_pair in enumerate(instance['qa_pairs']):
      ...
      _em = max([_compute_exact(a, p) for a in answers for p in prediction])
      _f1 = max([_compute_f1(a, p) for a in answers for p in prediction])

      if _em and False:
          print(_em, prediction, answers, full_key)
...
if __name__ == '__main__':
  ...
  try:
    with open(args.predictions, 'r') as handler:
      ...
      _p = collections.defaultdict(lambda: '')
  ...
  scores = compute_all_scores(hypotheses, asqa, target_keys=set(predictions.keys()))
